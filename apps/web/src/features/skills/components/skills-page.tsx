...
import { importMarkdownIntoEmptySheet } from '../skill-from-markdown'
import {
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        {skill ? (
          <SkillEditor
            ...
            onImportMarkdown={handleFileContentChange}
...
function SkillEditorHeader({
  ...
  const sheetContentRef = useRef(selectedContent)
  sheetContentRef.current = selectedContent

  const importMarkdownFile = (file: File) => {
    void importMarkdownIntoEmptySheet(file, () => sheetContentRef.current).then(
      (result) =>
        result.match({
          ok: onImportMarkdown,
