...
Inspect the existing Wrangler config before editing lifecycle state:

- New Workers should use declarative `exports`.
- Existing Workers may continue using the legacy ordered `migrations` array.
- `exports` and `migrations` are mutually exclusive. Converting an existing
  Worker requires replacing the full lifecycle configuration in one deliberate
  change; do not add `exports` beside old migrations.
- Every new namespace should use SQLite.
...
For declarative deletion, first remove the class export from code and remove
its binding, then keep a destructive tombstone:

```jsonc
{
  "exports": {
    "OldRoom": { "type": "durable-object", "state": "deleted" },
  },
}
```

For a Worker already using legacy migrations, never rewrite deployed entries.
Append a unique tag with exactly the lifecycle operation:

```jsonc
{
  "migrations": [
    { "tag": "v1", "new_sqlite_classes": ["Room"] },
    { "tag": "v2", "deleted_classes": ["OldRoom"] },
  ],
}
```

Deleting a class destroys its namespace and all stored data. Verify the exact
class and binding names before editing, and preserve or transfer required data
first. For declarative renames, the old entry uses `state: "renamed"` plus
`renamed_to`, and the destination also appears as a live entry.

Sources: Cloudflare “Durable Object class exports” and current Wrangler
configuration reference, last checked 2026-08-07.

### Per-object SQL schema migrations

Wrangler lifecycle configuration manages class namespaces; it does not migrate
tables inside each object. `PRAGMA user_version` is **not supported** in Durable
Objects SQLite storage. Use a `_sql_schema_migrations` table instead:
