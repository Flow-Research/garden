...
New Workers should use declarative `exports`. Cloudflare reconciles live
classes and destructive tombstones during deploy:

```jsonc
{
  "exports": {
    "MyDO": { "type": "durable-object", "storage": "sqlite" },
    "Obsolete": { "type": "durable-object", "state": "deleted" },
    "OldName": {
      "type": "durable-object",
      "state": "renamed",
      "renamed_to": "NewName",
...
Existing Workers already using legacy migrations may keep them. Append unique
entries; never rewrite deployed history:
...
- Prefer `exports` for new Workers; use the legacy array only when continuing an existing migration history
...
Remove a deleted class from code and bindings before deploying its tombstone
or legacy `deleted_classes` entry. Source: current Cloudflare Durable Object
class exports and Wrangler configuration docs, checked 2026-08-07.
